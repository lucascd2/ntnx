...
def interactive_menu(client):
    ...
    while True:
        try:
            ...
            if source_uuid:
                print(f"  v: Browse VMs in source ({source_name})")
...
Examples:
  ...
  python3 list_move_environments.py --server 10.38.18.23 --username nutanix
  
  # Non-interactive mode (original behavior)
  python3 list_move_environments.py --server 10.38.18.23 --username nutanix --no-interactive
        """
    )
...
def read_credential_mapping(csv_file_path="./credential-mapping.csv"):
    ...
    try:
        ...
        print(f"❌ Error: Credential mapping file '{csv_file_path}' not found")
        print("Creating sample file...")
        
        # Create sample file
        sample_content = """servername,username,password
AutoDC VM,administrator,SecurePass123!
CL-API-Dev,root,DevPassword456
Move 6.0,admin,MovePass789"""
        
        try:
            with open(csv_file_path, 'w') as f:
                f.write(sample_content)
            print(f"✅ Created sample credential mapping file: {csv_file_path}")
            print("Please edit this file with your actual VM credentials and run again.")
        except Exception as e:
